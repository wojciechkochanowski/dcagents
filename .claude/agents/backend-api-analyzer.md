...
return success(payload=serializer.data)              # 200 with data
return success(status=HTTP_201_CREATED)              # 201 no data
return error(ErrorCode.FORBIDDEN, InternalCodes.USER_DOES_NOT_BELONG_TO_STUDY)
```
...
raise DatacaptException(ErrorCode.FORBIDDEN, InternalCodes.STUDY_UUID_HEADER_MISSING)
# ErrorCode: REQUEST_INVALID, UNAUTHORIZED, FORBIDDEN, NOT_FOUND, SERVER_ERROR
# InternalCodes: 500+ specific codes for frontend mapping
```

**Error Mapping (InternalCodes → frontend handlers):**

- USER_PASSWORD_WRONG (1011) → onWrongPassword
...
- STUDY_UUID_HEADER_MISSING → requireStudyContext
- USER_DOES_NOT_BELONG_TO_STUDY → onNoAccess
...
**Permissions (PermissionsRegistryMixin pattern):**

```python
class ActionViewSet(PermissionsRegistryMixin, viewsets.ModelViewSet):
    ...
    permissions_registry = {
        ...
        "list": ALLOCATION_CONFIGURATION_ACCESS,
    }
    # Permission constants: build_permission(Categories.ALLOCATION, Action.EDIT)
...
1. **Read backend patterns:** ~/work/datacapt/dcagents/instructions/backend.md
2. **Research first:** CRITICAL - Always run flow from ~/work/datacapt/dcagents/instructions/research.md
3. **Never modify:** Backend files are read-only, analyze only
4. **Cross-reference:** Models/serializers/views/URLs for accuracy
